...
# Fotograafide normaliseerimiskaart (initsiaal -> täisnimi, kus selge)
FOTOGRAAF_MAPPING = {
...
def load_kihelkond_geojson():
    urls = [
        ...
        "https://raw.githubusercontent.com/tormi/Eesti-ajaloolised-halduspiirid-20ndal-sajandil/master/kihelkonnad.json",
    ]
    for url in urls:
        try:
...
if len(valitud_marksona) > 1:
    marksona_loogika = st.sidebar.radio(
        ...
        help="VÕI: foto, millel on mõni valitud märksõnadest. JA: foto, millel on KÕIK valitud märksõnad."
    )
else:
    marksona_loogika = "VÕI – vähemalt üks"

# Normalised names for filter
fotograafid_norm = sorted(fotod["Fotograaf (normaliseeritud)"].dropna().unique().tolist())
valitud_fotograaf = st.sidebar.multiselect("Fotograaf", fotograafid_norm, placeholder="Kõik fotograafid")

top_isikud = isikud["Isik"].value_counts().head(60).index.tolist()
valitud_isik = st.sidebar.multiselect("Isik pildil", top_isikud, placeholder="Kõik isikud")
...
if valitud_marksona:
    if marksona_loogika == "JA – kõik korraga":
        # AND: foto peab sisaldama KÕIKI valitud märksõnu
        pids = None
        ...
        # OR: foto peab sisaldama vähemalt ühte
        pids = marksoned[marksoned["Märksõna"].isin(valitud_marksona)]["PID"].unique()
        df = df[df["PID"].isin(pids)]

if valitud_isik:
    isik_pids = isikud[isikud["Isik"].isin(valitud_isik)]["PID"].unique()
...
with tab1:
    ...
    if "Kihelkonnapõhine" in asukoha_valik:
        ...
        if val_kihel != "—":
            ...
            with col_kd1:
                ft = df_kihel["Fotograaf (normaliseeritud)"].value_counts().head(8)
                if len(ft) > 0:
                    st.markdown("**Fotograafid**")
                    st.dataframe(ft.reset_index().rename(columns={"index": "Fotograaf", "Fotograaf (normaliseeritud)": "Arv"}), hide_index=True, use_container_width=True)
            with col_kd2:
                ms_kihel = marksoned[marksoned["PID"].isin(df_kihel["PID"])]["Märksõna"].value_counts().head(8)
                if len(ms_kihel) > 0:
                    st.markdown("**Top märksõnad**")
                    st.dataframe(ms_kihel.reset_index().rename(columns={"index": "Märksõna", "Märksõna": "Arv"}), hide_index=True, use_container_width=True)
...
with tab3:
    ...
    with col_m2:
        ...
        if ms_in:
            ms_tr = mf[mf["Märksõna"].str.lower() == ms_in.lower()].copy()
            # Merge aasta from df (not from marksoned, which may lack Aasta col)
            ms_tr = ms_tr.merge(df[["PID", "Aasta"]].drop_duplicates(), on="PID", how="left")
