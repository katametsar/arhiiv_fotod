...
with tab1:
    ...
    def naita_fotopunkte(df_piirkond, pealkiri, lisa_asustus_piirid=False):
        df_pts = df_piirkond[
        ...
        # Lisa asustusüksuse piirid kui soovitud
        if lisa_asustus_piirid:
            geojson_ay = load_geojson("asustusyksus.geojson")
            if geojson_ay:
                # Filtreeri ainult need asustusüksused mis jäävad vaadeldavale alale
                ...
                for feature in geojson_ay["features"]:
                    ...
                    for coords in coords_list:
                        lons = [c[0] for c in coords]
                        lats = [c[1] for c in coords]
                        # Näita ainult piirkonnas olevaid
    ...
    if "Kihelkonnapõhine" in asukoha_valik:
        ...
        if val_kihel != "—":
            ...
            naita_fotopunkte(df_kihel, f"Fotod – {val_kihel}", lisa_asustus_piirid=True)
