...
# ── Dünaamilised valikud (filtrite-tundlik) ───────────────────────────────────
...
# ── Sidebar – kõigepealt "kõik" filtrid, siis dünaamilised valikud ────────────
...
# Märksõnad – alati täisloend, aga näitame arvu praeguse filtri kontekstis
...
# Esialgu filtreeri ilma fotograafi ja isikuta, et saada dünaamilised valikud
...
# Dünaamiline fotograafiloend – ainult need, kellel on tulemusi praeguste filtritega
...
# Dünaamiline isikuloend – ainult need, kellel on tulemusi
pids_ilma_isik = set(df_ilma_ft_isik["PID"].unique())
isikud_saadaval_series = (
    isikud[isikud["PID"].isin(pids_ilma_isik)]["Isik"].value_counts().head(80)
)
isikud_saadaval = isikud_saadaval_series.index.tolist()
valitud_isik = st.sidebar.multiselect(
    "Isik pildil", isikud_saadaval, placeholder="Kõik isikud"
)

# Lõplik filtreeritud df
df = get_filtered_df(
    fotod, marksoned, isikud,
    aasta_vahemik, valitud_zanr,
    valitud_marksona, marksona_loogika,
...
with tab1:
    ...
    if "Kihelkonnapõhine" in asukoha_valik:
        ...
        st.markdown(
            "💡 **Tänapäevase haldusjaotuse GeoJSON failid** (vald, maakond) leiab "
            "[buildig/EHAK GitHubist](https://github.com/buildig/EHAK/tree/master/geojson). "
            "Lae alla `vald.geojson` ja `maakond.geojson` ning pane oma reposse."
        )
...
with tab3:
    ...
    with col_m2:
        ...
        if ms_in:
            ...
            # Võta Aasta ainult fotod-tabelist, väldi konflikti marksoned.Aasta-ga
            ms_tr = ms_tr.merge(
