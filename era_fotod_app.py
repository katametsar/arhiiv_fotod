...
def load_data():
    ...
    if not ml_clip.empty and "PID" in ml_clip.columns and "PID" in fotod.columns:
        ...
        ] if c in ml_clip.columns]
        for c in clip_cols:
            if c != "PID" and c in fotod.columns:
                fotod = fotod.drop(columns=[c])
        fotod = fotod.merge(ml_clip[clip_cols].drop_duplicates(subset=["PID"]), on="PID", how="left")
...
# Uuenda pärast märksõna kategooriat
zanr_opts, ms_opts, mk_opts, ft_opts, isik_opts = get_available_options(
    fotod, marksoned, isikud, aasta_vahemik,
    st.session_state["valitud_zanr"], st.session_state["valitud_marksona"],
    st.session_state["marksona_loogika_radio"], st.session_state["valitud_fotograaf"],
...
with tab5:
    ...
    if not has_clip and not has_manual:
        ...
        c3.metric("Käsitsi kategooriaga",
                  f"{ml_df['Märksõna kategooria'].notna().sum():,}" if "Märksõna kategooria" in ml_df.columns else "0")
        st.caption("Need arvud ei peagi olema samad: CLIP ennustused on ainult hindamiseks käsitsi kategooriaga "
                   "piltidel, ülejäänud fotodel võib olla originaalmärksõnu, aga mitte 19-kategooria käsitsi klastrit.")
        c4.metric(
            "Keskmine top1–top2 vahe",
            f"{ml_df['confidence_margin_top1_top2'].mean():.3f}"
            if "confidence_margin_top1_top2" in ml_df.columns and ml_df["confidence_margin_top1_top2"].notna().any()
            else "?"
        )
        ...
        if has_clip and has_manual:
            ...
            st.warning("Kui kattuvus on siin väga madal, siis kõige tõenäolisem põhjus ei ole mudel, vaid "
                       "kategoorianimede/andmeveeru sobimatus: CLIP-i `pred_top1` peab olema täpselt samas "
                       "19 kategooria sõnastikus nagu käsitsi `Märksõna kategooria`.")
            ...
            if not eval_df.empty:
                ...
                eval_df["top3_kattub"] = eval_df.apply(
                    lambda r: category_match(r, pred_cols=["pred_top1", "pred_top2", "pred_top3"]), axis=1)
                eval_df["top5_kattub"] = eval_df.apply(
                    lambda r: category_match(r, pred_cols=["pred_top1", "pred_top2", "pred_top3", "pred_top4", "pred_top5"]), axis=1)
        ...
        if has_clip:
            ...
            score_cols = [c for c in ["pred_top1_score", "confidence_margin_top1_top2",
                                       "ML top3 koondskoor", "ML top5 koondskoor"] if c in ml_df.columns]
            if score_cols:
                st.caption("Top1 skoor üksi ei ole väga hea kvaliteedimõõdik. Praktilisem on vaadata koos "
                           "top1–top2 vahet ning seda, kas õige/sobiv kategooria ilmub top3 või top5 hulka.")
        ...
        cols_ml = [c for c in [
            "PID", "failinimi", "Sisu kirjeldus", "Märksõna kategooria", "Originaal märksõnad",
            "pred_top1", "pred_top2", "pred_top3", "pred_top4", "pred_top5",
        ...
        st.dataframe(ml_show[cols_ml].head(500), use_container_width=True, hide_index=True, height=420)
